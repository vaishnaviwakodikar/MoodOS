...
//   text-transform: uppercase; color: var(--sage2); margin-bottom: 8px;
...
//   font-size: clamp(22px, 2.2vw, 28px); font-weight: 300; font-style: italic;
//   letter-spacing: -1px; line-height: 1.05; color: #fff; margin-bottom: 4px;
// }
// .at-h1 em { color: var(--sage2); }
// .at-sub { font-size: 10px; color: rgba(255,255,255,.35); }

// /* Stats */
// .at-stats { display: flex; flex-direction: column; gap: 6px; flex: 1; min-height: 0; overflow: hidden; }
// .at-stat {
...
// }
// .at-stat::after {
//   content: attr(data-n); position: absolute; bottom: -6px; right: 6px;
//   font-family: 'Fraunces', serif; font-size: 36px; opacity: .06;
//   pointer-events: none; line-height: 1;
// }
// .at-stat-lbl { font-size: 7.5px; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; color: rgba(255,255,255,.35); margin-bottom: 3px; }
// .at-stat-val { font-family: 'Fraunces', serif; font-size: clamp(18px,1.8vw,22px); font-weight: 300; letter-spacing: -1px; line-height: 1; margin-bottom: 1px; }
// .at-stat-sub { font-size: 9px; color: rgba(255,255,255,.28); }
// .at-bar { height: 3px; border-radius: 999px; background: rgba(255,255,255,.1); overflow: hidden; margin-top: 6px; }
...
//   flex-shrink: 0;
// }
// .at-footer-lbl { font-size: 7px; letter-spacing: 2.5px; text-transform: uppercase; color: rgba(255,255,255,.28); margin-bottom: 3px; }
// .at-footer-txt { font-family: 'Fraunces', serif; font-style: italic; font-size: 11px; font-weight: 300; color: rgba(255,255,255,.5); line-height: 1.5; }
...
// }
// .at-monlabel { font-family: 'Fraunces', serif; font-size: 17px; font-weight: 300; font-style: italic; color: var(--ink); letter-spacing: -.5px; }
// .at-monbtns  { display: flex; gap: 5px; }
// .at-monarrow { width: 26px; height: 26px; border-radius: 7px; border: 1px solid var(--line); background: var(--card); display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--ink3); font-size: 11px; transition: .15s; }
...
// .at-content::-webkit-scrollbar-track { background: transparent; }

// /* Calendar */
// .at-cal { background: var(--card); border: 1px solid var(--line); border-radius: 12px; padding: 12px; }
// .at-cal-days { display: grid; grid-template-columns: repeat(7,1fr); gap: 2px; margin-bottom: 3px; }
// .at-cal-day-lbl { text-align: center; font-size: 8px; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; color: var(--ink4); padding: 3px 0; }
// .at-cal-grid { display: grid; grid-template-columns: repeat(7,1fr); gap: 3px; }

// /* KEY FIX: use fixed height instead of aspect-ratio so calendar fits in viewport */
// .at-day {
//   height: 32px;
...
// .at-day-num { font-size: 10px; font-weight: 500; color: var(--ink2); line-height: 1; }
// .at-day-dot { width: 3px; height: 3px; border-radius: 50%; }
// .at-day.future { opacity: .3; cursor: not-allowed; pointer-events: none; }

// /* Two-col bottom */
// .at-row2 { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }

// /* Log strip */
// .at-card { background: var(--card); border: 1px solid var(--line); border-radius: 12px; padding: 12px; }
// .at-sec-title { font-size: 8px; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; color: var(--ink3); margin-bottom: 10px; display: flex; align-items: center; gap: 6px; }
// .at-sec-title i { font-size: 11px; color: var(--sage); }

// .at-status-row { display: flex; gap: 5px; margin-bottom: 8px; }
// .at-status-btn {
...
// .at-date-row { display: flex; align-items: center; gap: 6px; margin-bottom: 8px; }
// .at-date-in {
...
// .at-save-btn { width: 100%; padding: 8px; border-radius: 8px; border: none; font-family: 'Fraunces', serif; font-size: 12px; font-style: italic; font-weight: 300; cursor: pointer; transition: .18s; background: var(--ink); color: #fff; }
...
// .at-toast { padding: 5px 10px; border-radius: 6px; font-size: 10px; font-style: italic; font-family: 'Fraunces', serif; background: var(--sage3); border: 1px solid var(--sage2); color: var(--sage); text-align: center; margin-top: 8px; }

// /* History */
// .at-hist-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
// .at-hist-pct  { font-family: 'Fraunces', serif; font-size: 13px; font-weight: 300; color: var(--sage); font-style: italic; }
// .at-hist-inner { max-height: 160px; overflow-y: auto; overflow-x: hidden; }
// .at-hist-inner::-webkit-scrollbar { width: 2px; }
// .at-hist-inner::-webkit-scrollbar-thumb { background: var(--ink4); border-radius: 99px; }

// .at-hist-item { display: flex; align-items: center; gap: 8px; padding: 5px 7px; border-radius: 8px; margin-bottom: 3px; transition: .15s; }
// .at-hist-item:hover { background: var(--cream); }
// .at-hist-icon { width: 24px; height: 24px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 11px; flex-shrink: 0; }
...
// .at-hist-badge { padding: 2px 7px; border-radius: 999px; font-size: 8.5px; font-weight: 600; white-space: nowrap; flex-shrink: 0; }
// .at-hist-del  { background: none; border: none; cursor: pointer; color: var(--ink4); font-size: 11px; padding: 2px 4px; border-radius: 4px; transition: .15s; flex-shrink: 0; }
...
//   .at-side { width: 180px; padding: 18px 14px; }
//   .at-row2  { grid-template-columns: 1fr; }
// }
// @media (max-width: 640px) {
//   .at { flex-direction: column; }
//   .at-side { width: 100%; height: auto; flex-direction: row; flex-wrap: wrap; padding: 12px; gap: 10px; overflow: visible; }
//   .at-side-footer { display: none; }
//   .at-stats { flex-direction: row; flex-wrap: wrap; overflow: visible; }
//   .at-stat { flex: 1; min-width: 100px; }
//   .at-main { height: 0; flex: 1; }
// }
// `
...
//             {[
//               { label: 'attendance', value: `${pct}%`,      sub: `${present} of ${total} days`, color: '#a8c9ae', n: '%',  bar: true },
//               { label: 'present',    value: String(present), sub: 'days present',               color: '#a8c9ae', n: '✓',  bar: false },
//               { label: 'absent',     value: String(absent),  sub: 'days absent',                color: '#e8a0b0', n: '✗',  bar: false },
//               { label: 'holidays',   value: String(holiday), sub: 'days off',                   color: '#f5ddb4', n: '☀',  bar: false },
//             ].map((s, i) => (
//               <motion.div key={s.label} className="at-stat" data-n={s.n}
//                 initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
//                 transition={{ delay: .05 * i + .1 }}>
...
//               <button className="at-monarrow" onClick={() => navMonth(-1)}><i className="ti ti-chevron-left" /></button>
//               <button className="at-monarrow" onClick={() => navMonth(1)} disabled={isCurrentMonth}><i className="ti ti-chevron-right" /></button>
//             </div>
//           </div>
...
//                 {Array.from({ length: firstDay }).map((_,i) => <div key={`e-${i}`} className="at-day empty" />)}
...
//                       style={{ background: cfg ? cfg.bg : 'transparent', borderColor: cfg ? cfg.border : isToday ? 'var(--sage2)' : 'transparent' }}
...
//                       <span className="at-day-num" style={{ color: cfg ? cfg.c : isToday ? 'var(--sage)' : 'var(--ink2)' }}>{day}</span>
...
//                   {selDate && (
//                     <span style={{ fontSize: 10, color: 'var(--ink3)', whiteSpace: 'nowrap', flexShrink: 0 }}>
...
//                 <button className={`at-save-btn${selStatus?' ready':''}`} onClick={saveAttendance} disabled={!selStatus||saving}>
...
//               {/* History */}
//               <motion.div className="at-card"
//                 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .28 }}>
...
//                             exit={{ opacity: 0, x: 8 }} transition={{ delay: i*.025 }}>
...
//                               <div className="at-hist-date">{d.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
//                               <div className="at-hist-day">{d.toLocaleDateString('en-IN', { weekday: 'long' })}</div>
//                             </div>
//                             <span className="at-hist-badge" style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, color: cfg.c }}>{cfg.label}</span>
//                             <button className="at-hist-del" onClick={() => deleteRecord(rec.id)}><i className="ti ti-trash" /></button>
//                           </motion.div>
//                         )
//                       })}
//                     </AnimatePresence>
