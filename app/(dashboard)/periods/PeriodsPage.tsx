...
function OnboardingFlow({ onComplete }: { onComplete: (profile: CycleProfile) => void }) {
  ...
  const [profile, setProfile] = useState<Partial<CycleProfile>>({
    cycle_length: 28, period_length: 5, cycle_regularity: 'regular',
    has_pcos_pcod: false, pcos_type: null, on_birth_control: false,
    birth_control_type: null, trying_to_conceive: false, age: null,
    last_period_date: toYMD(new Date()), notes: null,
  })
...
const cravingFoods = [
  { img: 'https://images.unsplash.com/photo-1548907040-4d42bde4e6ab?w=300&q=80', label: 'chocolate', emoji: '🍫', vibe: 'serotonin boost bestie', c: '#7a3a1a', bg: 'rgba(122,58,26,0.08)', border: 'rgba(122,58,26,0.15)' },
  { img: 'https://images.unsplash.com/photo-1513104890138-7c749659a591?w=300&q=80', label: 'pizza', emoji: '🍕', vibe: 'cheesy comfort, always', c: '#c05828', bg: 'rgba(192,88,40,0.08)', border: 'rgba(192,88,40,0.15)' },
  { img: 'https://images.unsplash.com/photo-1551024506-0bccd828d307?w=300&q=80', label: 'ice cream', emoji: '🍨', vibe: 'cold + sweet = therapy', c: '#9b5ec8', bg: 'rgba(155,94,200,0.08)', border: 'rgba(155,94,200,0.15)' },
  { img: 'https://images.unsplash.com/photo-1612203985729-70726954388c?w=300&q=80', label: 'ramen', emoji: '🍜', vibe: 'warm hug in a bowl', c: '#c08020', bg: 'rgba(192,128,32,0.08)', border: 'rgba(192,128,32,0.15)' },
  { img: 'https://images.unsplash.com/photo-1562376552-0d160a2f238d?w=300&q=80', label: 'fries', emoji: '🍟', vibe: 'salty & satisfying', c: '#b8860b', bg: 'rgba(184,134,11,0.08)', border: 'rgba(184,134,11,0.15)' },
  { img: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=300&q=80', label: 'cake', emoji: '🎂', vibe: 'because you deserve it', c: '#d4607a', bg: 'rgba(212,96,122,0.08)', border: 'rgba(212,96,122,0.15)' },
]
...
function PeriodCravingsCard() {
  ...
  useEffect(() => {
    const fetchCravingClicks = async () => {
      ...
      if (error) { logSupabaseError('fetchCravingClicks failed:', error); return }
  ...
  const handleFoodClick = async (label: string) => {
    ...
    if (authError || !user) { logSupabaseError('handleFoodClick: no user', authError); return }
  ...
  return (
    <motion.div
      ...
      style={{
        borderRadius: '20px', background: 'linear-gradient(145deg, #fff5f7 0%, #f8f3fc 100%)',
        border: '1.5px solid rgba(212,96,122,0.15)', padding: '18px 16px', marginTop: '6px', overflow: 'hidden',
      }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
        <span style={{ fontSize: '24px' }}>🫶</span>
        <div>
          <div style={{ fontSize: '13px', fontWeight: 700, color: '#d4607a', lineHeight: 1.2 }}>your body is craving…</div>
          <div style={{ fontSize: '11px', color: '#b09aa4', marginTop: '2px' }}>tap whichever you reach for 🌸</div>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '14px' }}>
        {cravingFoods.map((food) => {
          ...
          return (
            <button key={food.label} onClick={() => handleFoodClick(food.label)}
              style={{
                borderRadius: '16px', overflow: 'hidden',
                border: `1.5px solid ${isClicked ? food.c : food.border}`,
                background: food.bg, display: 'flex', flexDirection: 'column',
                width: '100%', padding: 0, margin: 0, cursor: 'pointer',
              ...
              <div style={{ position: 'relative', width: '100%', paddingTop: '85%', overflow: 'hidden' }}>
                <img src={food.img} alt={food.label} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                <div style={{ position: 'absolute', top: 6, right: 6, fontSize: '15px', background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(4px)', borderRadius: '50%', width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                  {food.emoji}
                </div>
                {isClicked && (
                  <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                    style={{ position: 'absolute', top: 6, left: 6, width: 24, height: 24, background: food.c, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.18)' }}>
              ...
              <div style={{ padding: '8px 10px 9px' }}>
                ...
                <div style={{ fontSize: '9.5px', color: '#b09aa4', marginTop: '2px', lineHeight: 1.4 }}>{isClicked ? 'craved it ✓' : food.vibe}</div>
              </div>
            </button>
          )
        })}
      </div>
      <div style={{ padding: '10px 12px', borderRadius: '12px', background: 'rgba(155,126,200,0.07)', border: '1px solid rgba(155,126,200,0.15)', fontSize: '11px', color: '#9b7ec8', lineHeight: 1.6, textAlign: 'center' }}>
...
export default function PeriodsPage() {
  ...
  useEffect(() => {
    fetchEntries(); fetchProfile(); fetchSexLogs(); fetchPainLogs(); fetchDailySymptoms()
  }, [])

  const fetchProfile = async () => {
    const { data, error } = await supabase.from('cycle_profiles').select('*').single()
    if (error) { logSupabaseError('fetchProfile failed:', error); return }
    if (data) {
      setCycleProfile({
        ...data,
  ...
  const fetchEntries = async () => {
    const { data, error } = await supabase.from('period_entries').select('*').order('start_date', { ascending: false }).limit(12)
    if (error) { logSupabaseError('fetchEntries failed:', error); return }
  ...
  const fetchSexLogs = async () => {
    ...
    if (error) { logSupabaseError('fetchSexLogs failed:', error); return }
  ...
  const fetchPainLogs = async () => {
    ...
    if (error) { logSupabaseError('fetchPainLogs failed:', error); return }
  ...
  const fetchDailySymptoms = async () => {
    ...
    if (error) { logSupabaseError('fetchDailySymptoms failed:', error); return }
  ...
  const handleLogDailySymptom = async () => {
    ...
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    if (authError || !user) { logSupabaseError('handleLogDailySymptom: no user', authError); setSavingSymptom(false); return }
    ...
    if (error) { logSupabaseError('handleLogDailySymptom: upsert failed:', error); setSavingSymptom(false); return }
    setDailySelSymptoms([]); setDailyMood(null); setDailyEnergy(3); setDailyNotes('')
    setSuccessSymptom(true); setTimeout(() => setSuccessSymptom(false), 3000)
    setSavingSymptom(false); fetchDailySymptoms()
  }
  ...
  const openEditSymptom = (s: DailySymptom) => {
    setEditingSymptom(s); setEditDDate(s.date); setEditDSymptoms(s.symptoms ?? [])
    setEditDMood(s.mood ?? null); setEditDEnergy(s.energy ?? 3); setEditDNotes(s.notes ?? '')
  ...
  const handleSaveEditSymptom = async () => {
    ...
    const { error } = await (supabase.from('daily_symptoms') as any).update({
      date: editDDate, symptoms: editDSymptoms, mood: editDMood, energy: editDEnergy, notes: editDNotes.trim() || null,
    }).eq('id', editingSymptom.id)
    if (error) logSupabaseError('handleSaveEditSymptom failed:', error)
    setSavingEditSymptom(false); closeEditSymptom(); fetchDailySymptoms()
  }
  ...
  const handleDeleteSymptom = async () => {
    ...
    closeEditSymptom(); fetchDailySymptoms()
  }

  const handleOnboardingComplete = async (profile: CycleProfile) => {
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    if (authError || !user) { logSupabaseError('handleOnboardingComplete: no user', authError); return }
    const { error } = await (supabase as any).from('cycle_profiles').upsert({ ...profile, user_id: user.id })
    if (error) { logSupabaseError('handleOnboardingComplete: upsert failed:', error); return }
    setCycleProfile(profile); localStorage.setItem('cycle_onboarded', '1'); setShowOnboarding(false)
  }
  ...
  const handleLog = async () => {
    ...
    setSaving(true)
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    if (authError || !user) { logSupabaseError('handleLog: no user', authError); setSaving(false); return }

    const { data, error } = await (supabase.from('period_entries') as any).insert({
      user_id: user.id,
    ...
    if (error) { logSupabaseError('handleLog: insert failed:', error); setSaving(false); return }
  ...
  const handleArrivalConfirm = async (flow: Flow, date: string, symptoms: string[], mood: string | null, cravings: string[]) => {
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    if (authError || !user) { logSupabaseError('handleArrivalConfirm: no user', authError); return }
    const { error } = await (supabase.from('period_entries') as any).insert({
      user_id: user.id, start_date: date, end_date: null, flow, symptoms, mood, cravings, notes: null,
    })
    if (error) { logSupabaseError('handleArrivalConfirm: insert failed:', error); return }
    setArrivalSnoozed(true); fetchEntries()
  }
  ...
  const handleLogPain = async () => {
    ...
    setSavingPain(true)
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    if (authError || !user) { logSupabaseError('handleLogPain: no user', authError); setSavingPain(false); return }
    const { error } = await supabase.from('pain_logs').insert({
      user_id: user.id, date: painDate, type: painType, severity: painSeverity,
      duration_hours: painDuration ? Number(painDuration) : null,
      relief_used: painRelief.join(', ') || null, notes: painNotes.trim() || null,
    })
    if (error) { logSupabaseError('handleLogPain: insert failed:', error); setSavingPain(false); return }
    setSuccessPain(true)
  ...
  const handleLogSex = async () => {
    ...
    try {
      ...
      setSuccessSex(true); setSexNotes(''); setSexDate(toYMD(new Date()))
      setTimeout(() => setSuccessSex(false), 3000); fetchSexLogs()
    } catch (err) { logSupabaseError('Unexpected error:', err) }
    finally { setSavingSex(false) }
  }

  const openEdit = (e: Entry) => {
    setEditingEntry(e); setEditFlow(e.flow); setEditSymptoms(e.symptoms ?? [])
    setEditMood(e.mood ?? null); setEditCravings(e.cravings ?? [])
    setEditStart(e.start_date); setEditEnd(e.end_date ?? ''); setEditNotes(e.notes ?? '')
...
const handleSaveEdit = async () => {
  ...
  const openEditPain = (p: PainLog) => {
    setEditingPain(p); setEditPainType(p.type); setEditPainSeverity(p.severity)
    ...
    setEditPainNotes(p.notes ?? ''); setEditPainDate(p.date); setConfirmDeletePain(false)
  }
  ...
  const handleSaveEditPain = async () => {
    ...
    const { error } = await supabase.from('pain_logs').update({
      ...
      relief_used: editPainRelief.join(', ') || null, notes: editPainNotes.trim() || null,
    }).eq('id', editingPain.id)
    if (error) logSupabaseError('handleSaveEditPain failed:', error)
    setSavingEditPain(false); closeEditPain(); fetchPainLogs()
  }
  ...
  const handleDeletePain = async () => {
    ...
    closeEditPain(); fetchPainLogs()
  }

  const openEditSex = (s: SexLog) => {
    setEditingSex(s); setEditSexDate(s.date); setEditSexProtected(s.protected ?? true)
    setEditSexNotes(s.notes ?? ''); setConfirmDeleteSex(false)
  }
  ...
  const handleSaveEditSex = async () => {
    ...
    const { error } = await supabase.from('sex_log').update({
      date: editSexDate, protected: editSexProtected, notes: editSexNotes.trim() || null,
    }).eq('id', editingSex.id)
    if (error) logSupabaseError('handleSaveEditSex failed:', error)
    setSavingEditSex(false); closeEditSex(); fetchSexLogs()
  }
  ...
  const handleDeleteSex = async () => {
    ...
    closeEditSex(); fetchSexLogs()
  }

  const openEditProfile = () => { setEditProfile({ ...cycleProfile }); setEditingProfile(true) }
  const closeEditProfile = () => setEditingProfile(false)
  const updateEditProfile = (k: keyof CycleProfile, v: any) => setEditProfile(p => ({ ...p, [k]: v }))

  const handleSaveEditProfile = async () => {
    if (!editProfile) return
    setSavingEditProfile(true)
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    if (authError || !user) { logSupabaseError('handleSaveEditProfile: no user', authError); setSavingEditProfile(false); return }
    const { error } = await (supabase as any).from('cycle_profiles').upsert({ ...editProfile, user_id: user.id })
    if (error) { logSupabaseError('handleSaveEditProfile: upsert failed:', error); setSavingEditProfile(false); return }
    setCycleProfile(editProfile as CycleProfile); setSavingEditProfile(false); closeEditProfile()
  }
          ...
          {cycleProfile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
              ...
              <button className={styles['pt-profile-pill']} onClick={() => setActiveTab('profile')}>
                <i className="ti ti-settings" style={{ color: '#d4607a' }} />edit profile
              </button>
              <button onClick={() => setShareOpen(true)} style={{
                display: 'flex', alignItems: 'center', gap: '6px',
                background: 'linear-gradient(135deg, #ff3d8b, #ff6bb5)', border: 'none',
                borderRadius: '50px', padding: '7px 16px', fontSize: '12px', fontWeight: 500,
...
share
        ...
        <AnimatePresence>
          {lastStart && (
            <PeriodArrivalBanner
              daysUntilNext={daysUntilNext} predictedDate={nextPredicted}
              onConfirm={handleArrivalConfirm} onSnooze={() => setArrivalSnoozed(true)}
        ...
        <div className={styles['pt-stats']}>
          {statCards.map((s, i) => (
            <motion.div key={s.label} className={styles['pt-stat']}
              ...
              initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 * i }}>
        ...
        <div className={styles['pt-tabs']}>
          {tabs.map(t => {
            ...
            return (
              <button key={t.key} className={styles['pt-tab']} onClick={() => setActiveTab(t.key)}
                style={{
        ...
        <AnimatePresence mode="wait">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div className={styles['pt-card']}>
                  ...
                  <textarea className={styles['pt-input']} value={notes} rows={3}
                    onChange={e => setNotes(e.target.value)} placeholder="anything on your mind..." style={{ resize: 'none' }} />
                ...
                <div className={styles['pt-card']}>
                  ...
                  <div className={styles['pt-chips']}>
                    {symptomList.map(s => {
                      ...
                      return (
                        <button key={s.label} className={styles['pt-chip']} onClick={() => toggleSymptom(s.label)}
                          style={{
                ...
                <div className={styles['pt-card']}>
                  ...
                  <div className={styles['pt-chips']}>
                    {cravingOpts.map(c => {
                      ...
                      return (
                        <button key={c.label} className={styles['pt-chip']} onClick={() => toggleCraving(c.label)}
                          style={{
                ...
                <div className={styles['pt-card']}>
                  ...
                  <div className={styles['pt-mood-row']}>
                    {moodOpts.map(m => {
                      ...
                      return (
                        <button key={m.label} className={styles['pt-mood-btn']} onClick={() => setSelMood(active ? null : m.label)}
                          style={{
                            background: active ? '#fde8ee' : 'transparent',
                            borderColor: active ? '#e8a0b0' : 'rgba(212,96,122,0.15)',
                            color: active ? m.c : '#b09aa4',
                        ...
                        return (
                          <motion.div key={e.id} className={styles['pt-hitem']}
                            ...
                            <button onClick={() => openEdit(e)}
                              style={{ width: 30, height: 30, borderRadius: '50%', border: '1.5px solid rgba(212,96,122,0.18)', background: '#fdf8fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#d4607a', flexShrink: 0, transition: 'all 0.18s ease' }}
            ...
            <motion.div key="symptoms" className={styles['pt-grid']}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div className={styles['pt-card']}>
                  ...
                  <input type="date" className={styles['pt-input']} value={symptomDate} onChange={e => setSymptomDate(e.target.value)} />
                  ...
                  <div className={styles['pt-chips']} style={{ marginBottom: '14px' }}>
                    {symptomList.map(s => {
                      ...
                      return (
                        <button key={s.label} className={styles['pt-chip']} onClick={() => toggleDailySymptom(s.label)}
                          style={{ background: active ? '#f3edfb' : 'transparent', borderColor: active ? '#c9b8e8' : 'rgba(155,126,200,0.2)', color: active ? '#4a2a80' : '#b09aa4' }}>
                          <i className={`ti ${s.icon}`} style={{ color: active ? '#9b7ec8' : '#b09aa4' }} />{s.label}
                        </button>
                      )
                    })}
                  </div>
                  ...
                  <div className={styles['pt-mood-row']} style={{ marginBottom: '14px' }}>
                    {moodOpts.map(m => {
                      ...
                      return (
                        <button key={m.label} className={styles['pt-mood-btn']} onClick={() => setDailyMood(active ? null : m.label)}
                          style={{ background: active ? '#f3edfb' : 'transparent', borderColor: active ? '#c9b8e8' : 'rgba(155,126,200,0.15)', color: active ? m.c : '#b09aa4' }}>
                          <i className={`ti ${m.icon}`} style={{ color: active ? m.c : '#b09aa4' }} />{m.label}
                        </button>
                      )
                    })}
                  </div>
                  ...
                  <div className={styles['pt-severity']} style={{ marginBottom: '14px' }}>
                    {[1,2,3,4,5].map(n => (
                      <button key={n} onClick={() => setDailyEnergy(n)}
                        style={{ width: 36, height: 36, borderRadius: '10px', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '13px', fontFamily: 'DM Sans, sans-serif', background: dailyEnergy >= n ? `hsl(${140 + n * 15}, ${50 + n * 5}%, ${55 - n * 3}%)` : 'transparent', color: dailyEnergy >= n ? '#fff' : '#b09aa4', outline: dailyEnergy >= n ? 'none' : '1.5px solid rgba(155,126,200,0.2)', transition: 'all 0.15s' }}>
                        {n}
                      </button>
                    ))}
                  ...
                  <textarea className={styles['pt-input']} value={dailyNotes} rows={2}
                    onChange={e => setDailyNotes(e.target.value)} placeholder="anything on your mind..." style={{ resize: 'none' }} />
                  <button className={styles['pt-submit']} onClick={handleLogDailySymptom}
                    disabled={(!dailySelSymptoms.length && !dailyMood) || savingSymptom}
                    style={{ background: (dailySelSymptoms.length || dailyMood) ? 'linear-gradient(135deg, #9b7ec8, #d4607a)' : 'rgba(155,126,200,0.08)', color: (dailySelSymptoms.length || dailyMood) ? '#fff' : '#b09aa4' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '9px' }}>
                    {dailySymptoms.map((s, i) => (
                      <motion.div key={s.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                        style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '10px 12px', borderRadius: '13px', background: 'rgba(155,126,200,0.05)', border: '1px solid rgba(155,126,200,0.12)' }}>
                        <div style={{ width: 34, height: 34, borderRadius: '10px', flexShrink: 0, background: '#f3edfb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {s.symptoms?.length > 0 && (
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: s.notes ? '4px' : 0 }}>
                              {s.symptoms.map(sym => (
                                <span key={sym} style={{ padding: '2px 8px', borderRadius: '999px', fontSize: '10px', fontWeight: 600, background: 'rgba(155,126,200,0.1)', color: '#9b7ec8', border: '1px solid rgba(155,126,200,0.2)' }}>{sym}</span>
                              ))}
                            </div>
                          )}
                          {s.notes && <div style={{ fontSize: '11px', color: '#b09aa4', fontStyle: 'italic', marginTop: '3px' }}>{s.notes}</div>}
                        </div>
                        <button onClick={() => openEditSymptom(s)}
                          style={{ width: 30, height: 30, borderRadius: '50%', flexShrink: 0, border: '1.5px solid rgba(155,126,200,0.2)', background: '#faf8fe', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#9b7ec8', transition: 'all 0.18s ease' }}
            ...
            <motion.div key="sex" className={styles['pt-grid']}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div className={styles['pt-card']}>
                  ...
                  <div className={styles['pt-toggle-row']}>
                    <span className={styles['pt-toggle-lbl']}><i className="ti ti-shield-check" style={{ color: '#5a8c63' }} />protected?</span>
                  ...
                  {cycleProfile?.trying_to_conceive && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      style={{ padding: '12px 14px', borderRadius: '12px', marginBottom: '10px', background: 'rgba(155,126,200,0.08)', border: '1px solid rgba(155,126,200,0.2)', fontSize: '12px', color: '#9b7ec8', lineHeight: 1.5 }}>
                      <i className="ti ti-sparkles" style={{ marginRight: '6px' }} />
                      TTC tip: {phase === 'ovulation' ? "You're in your fertile window — best timing!" : phase === 'follicular' ? 'Approaching fertile window — good timing.' : 'Ovulation window is in a few days.'}
                    </motion.div>
                  )}
                  ...
                  <textarea className={styles['pt-input']} value={sexNotes} rows={2}
                    onChange={e => setSexNotes(e.target.value)} placeholder="e.g. pain, discomfort, notes..." style={{ resize: 'none' }} />
                    ...
                    {sexLogs.slice(0, 10).map((s, i) => (
                      <motion.div key={s.id} className={styles['pt-sex-row']}
                        ...
                        <span className={styles['pt-sex-badge']}
                          style={{ background: s.protected ? 'rgba(90,140,99,0.12)' : 'rgba(212,96,122,0.1)', color: s.protected ? '#5a8c63' : '#d4607a', border: `1px solid ${s.protected ? 'rgba(90,140,99,0.25)' : 'rgba(212,96,122,0.2)'}` }}>
                          {s.protected ? 'protected' : 'unprotected'}
                        </span>
                        <button onClick={() => openEditSex(s)}
                          style={{ width: 30, height: 30, borderRadius: '50%', border: '1.5px solid rgba(90,140,99,0.2)', background: '#f8fdf9', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#5a8c63', flexShrink: 0, transition: 'all 0.18s ease' }}
            ...
            <motion.div key="calendar" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }}>
              <div className={styles['pt-card']}>
                <div className={styles['pt-cal-nav']}>
                  <button className={styles['pt-cal-nav-btn']} onClick={() => setCalMonth(m => new Date(m.getFullYear(), m.getMonth() - 1, 1))}>
                    <i className="ti ti-chevron-left" />
                  </button>
                  <span className={styles['pt-cal-month']}>{calMonth.toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })}</span>
                  <button className={styles['pt-cal-nav-btn']} onClick={() => setCalMonth(m => new Date(m.getFullYear(), m.getMonth() + 1, 1))}>
                    <i className="ti ti-chevron-right" />
                  </button>
                </div>
                <div className={styles['pt-cal-grid']}>
                  {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d => <div key={d} className={styles['pt-cal-dow']}>{d}</div>)}
                  {[...Array(firstDay)].map((_, i) => <div key={`e-${i}`} className={`${styles['pt-cal-day']} ${styles.empty}`} />)}
                  {[...Array(daysInMon)].map((_, i) => {
                    ...
                    return (
                      <div key={day} className={cls} style={{ fontSize: '12px', color: isT && !iP && !iPr && !iOv ? '#d4607a' : undefined }}>
                        {day}
                      </div>
                    )
                  })}
          ...
          {/* ── INSIGHTS ── */}
          {activeTab === 'insights' && (
            <motion.div key="insights" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                    ...
                    {painTypes.map(pt2 => {
                      ...
                      return (
                        <div key={pt2.key} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          <div style={{ flex: 1, height: '6px', borderRadius: '999px', background: 'rgba(184,134,11,0.1)', overflow: 'hidden' }}>
                            <motion.div initial={{ width: 0 }} animate={{ width: `${(Number(avgSev) / 5) * 100}%` }} transition={{ delay: 0.2, duration: 0.6 }}
          ...
          {/* ── PROFILE ── */}
          {activeTab === 'profile' && (
            <motion.div key="profile" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
              style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div className={styles['pt-card']}>
                <p className={styles['pt-card-lbl']}><i className="ti ti-user" /> your cycle profile</p>
                {!cycleProfile ? (
                  <div className={styles['pt-empty']}>
                    <i className="ti ti-calendar-heart" />no profile yet
                  ...
                  <>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
                      {[
                        ...
                        <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', borderRadius: '14px', background: row.bg }}>
                          <i className={`ti ${row.icon}`} style={{ color: row.c, fontSize: '16px' }} />
                          <span style={{ fontSize: '13px', color: 'var(--ink)', flex: 1 }}>{row.label}</span>
                    ...
                    {cycleProfile.notes && (
                      <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(212,96,122,0.04)', border: '1px solid rgba(212,96,122,0.1)', fontSize: '13px', color: '#b09aa4', fontStyle: 'italic', marginBottom: '14px' }}>
                    ...
                    <button onClick={() => setShareOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'linear-gradient(135deg, #ff3d8b, #ff6bb5)', border: 'none', borderRadius: '50px', padding: '10px 20px', fontSize: '13px', fontWeight: 500, color: 'white', cursor: 'pointer', letterSpacing: '0.03em', boxShadow: '0 4px 16px rgba(255, 61, 139, 0.35)', transition: 'transform 0.2s ease, box-shadow 0.2s ease' }}
        ...
        <AnimatePresence>
          {editingEntry && (
            <OnboardingPortal>
              <motion.div className={styles['pt-onboard-overlay']} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <motion.div className={styles['pt-onboard-card']} style={{ maxWidth: 540 }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                    <div>
                      ...
                      <h2 className={styles['pt-onboard-title']} style={{ marginBottom: 0 }}>edit <span className={styles.accent}>cycle entry</span></h2>
                    </div>
                    <button onClick={closeEdit} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid #eddde3', background: '#fdf8fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#b09aa4', flexShrink: 0 }}>
                      <i className="ti ti-x" />
                    </button>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', maxHeight: '65vh', overflowY: 'auto', paddingRight: '4px' }}>
                        ...
                        {flows.map(f => {
                          ...
                          return (
                            <button key={f.key} className={styles['pt-flow-btn']}
                              ...
                              style={{ background: active ? f.bg : 'transparent', borderColor: active ? f.border : 'rgba(212,96,122,0.12)', color: active ? f.c : '#b09aa4' }}>
                              <i className="ti ti-droplet" style={{ color: active ? f.c : '#b09aa4' }} />{f.label}
                              <div className={styles['pt-flow-dots']}>
                                {f.key === 'spotting'
                                  ? <div className={styles['pt-flow-dot']} style={{ background: 'rgba(0,0,0,0.15)', width: '3px', height: '3px' }} />
                                  : [...Array(4)].map((_, i) => <div key={i} className={styles['pt-flow-dot']} style={{ background: i < f.dots ? f.c : 'rgba(0,0,0,0.08)' }} />)
                                }
                              </div>
                            </button>
                          )
                    ...
                    <div>
                      ...
                      <div className={styles['pt-chips']}>
                        {symptomList.map(s => {
                          ...
                          return (
                            <button key={s.label} className={styles['pt-chip']} onClick={() => toggleEditSymptom(s.label)}
                              style={{ background: active ? '#fde8ee' : 'transparent', borderColor: active ? '#e8a0b0' : 'rgba(212,96,122,0.15)', color: active ? '#7a1a35' : '#b09aa4' }}>
                              <i className={`ti ${s.icon}`} style={{ color: active ? '#d4607a' : '#b09aa4' }} />{s.label}
                            </button>
                          )
                        })}
                      </div>
                    ...
                    <div>
                      ...
                      <div className={styles['pt-chips']}>
                        {cravingOpts.map(c => {
                          ...
                          return (
                            <button key={c.label} className={styles['pt-chip']} onClick={() => toggleEditCraving(c.label)}
                              style={{ background: active ? '#fef8e7' : 'transparent', borderColor: active ? '#f5ddb4' : 'rgba(212,96,122,0.15)', color: active ? c.c : '#b09aa4' }}>
                              <i className={`ti ${c.icon}`} style={{ color: active ? c.c : '#b09aa4' }} />{c.label}
                            </button>
                          )
                        })}
                      </div>
                    ...
                    <div>
                      ...
                      <div className={styles['pt-mood-row']}>
                        {moodOpts.map(m => {
                          ...
                          return (
                            <button key={m.label} className={styles['pt-mood-btn']} onClick={() => setEditMood(active ? null : m.label)}
                              style={{ background: active ? '#fde8ee' : 'transparent', borderColor: active ? '#e8a0b0' : 'rgba(212,96,122,0.15)', color: active ? m.c : '#b09aa4' }}>
                              <i className={`ti ${m.icon}`} style={{ color: active ? m.c : '#b09aa4' }} />{m.label}
                            </button>
                          )
                        })}
                    ...
                    <div>
                      ...
                      <textarea className={styles['pt-input']} value={editNotes} rows={2}
                        onChange={e => setEditNotes(e.target.value)} placeholder="anything on your mind..." style={{ resize: 'none', marginBottom: 0 }} />
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px' }}>
                    <button className={styles['pt-submit']} onClick={handleSaveEdit} disabled={!editFlow || savingEdit}
                      style={{ background: editFlow ? 'linear-gradient(135deg, #d4607a, #9b7ec8)' : 'rgba(212,96,122,0.08)', color: editFlow ? '#fff' : '#b09aa4' }}>
                      {savingEdit ? 'saving...' : 'save changes ✨'}
                    </button>
                    {!confirmDelete ? (
                      <button onClick={() => setConfirmDelete(true)} style={{ width: '100%', padding: '11px', borderRadius: '999px', border: '1.5px solid rgba(212,96,122,0.2)', background: 'transparent', color: '#b09aa4', fontSize: '13px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>
                        <i className="ti ti-trash" style={{ marginRight: '6px' }} />delete this entry
                      </button>
                    ) : (
                      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                        style={{ padding: '14px 18px', borderRadius: '18px', background: 'rgba(212,96,122,0.06)', border: '1.5px solid rgba(212,96,122,0.2)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <p style={{ fontSize: '13px', color: '#8b1a35', margin: 0, fontWeight: 600, textAlign: 'center' }}>are you sure? this can&apos;t be undone.</p>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button onClick={() => setConfirmDelete(false)} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: '1.5px solid #eddde3', background: '#fdf8fa', color: '#b09aa4', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>cancel</button>
                          <button onClick={handleDeleteEntry} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: 'none', background: 'linear-gradient(135deg, #d4607a, #8b1a35)', color: 'white', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 700 }}>
                ...
                <motion.div className={styles['pt-onboard-card']} style={{ maxWidth: 480 }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                    <div>
                      ...
                      <h2 className={styles['pt-onboard-title']} style={{ marginBottom: 0 }}>edit <span style={{ color: '#b8860b' }}>pain log</span></h2>
                    </div>
                    <button onClick={closeEditPain} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid #eddde3', background: '#fdf8fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#b09aa4', flexShrink: 0 }}>
                      <i className="ti ti-x" />
                    </button>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', maxHeight: '65vh', overflowY: 'auto', paddingRight: '4px' }}>
                        ...
                        {painTypes.map(pt2 => {
                          ...
                          return (
                            <button key={pt2.key} className={styles['pt-pain-type-btn']}
                              ...
                              style={{ background: active ? pt2.bg : 'transparent', borderColor: active ? pt2.c : 'rgba(212,96,122,0.12)', color: active ? pt2.c : '#b09aa4' }}>
                              <i className={`ti ${pt2.icon}`} style={{ color: active ? pt2.c : '#b09aa4' }} />{pt2.label}
                            </button>
                          )
                        })}
                      </div>
                      ...
                      <div className={styles['pt-severity']}>
                        {[1,2,3,4,5].map(n => (
                          <button key={n} className={`${styles['pt-sev-btn']} ${editPainSeverity >= n ? styles.active : ''}`}
                            ...
                            style={{ background: editPainSeverity >= n ? `hsl(${340 - n * 18},${60 + n * 6}%,${60 - n * 4}%)` : 'transparent', borderColor: editPainSeverity >= n ? 'transparent' : 'rgba(212,96,122,0.15)' }}>
                            {n}
                          </button>
                        ))}
                        <span style={{ fontSize: '11px', color: '#b09aa4', marginLeft: '4px' }}>
                    ...
                    <div>
                      ...
                      <input type="number" className={styles['pt-input']} placeholder="e.g. 2" value={editPainDuration} onChange={e => setEditPainDuration(e.target.value)} />
                    </div>
                    <div>
                      <div className={styles['pt-input-lbl']}><i className="ti ti-first-aid-kit" /> relief used</div>
                      <div className={styles['pt-chips']} style={{ marginBottom: '4px' }}>
                        {reliefOptions.map(r => {
                          ...
                          return (
                            <button key={r} className={styles['pt-chip']} onClick={() => toggleEditPainRelief(r)}
                              style={{ background: active ? '#fef8e7' : 'transparent', borderColor: active ? '#f5ddb4' : 'rgba(212,96,122,0.15)', color: active ? '#5a3a00' : '#b09aa4' }}>
                              {r}
                            </button>
                          )
                    ...
                    <div>
                      ...
                      <textarea className={styles['pt-input']} value={editPainNotes} rows={2}
                        onChange={e => setEditPainNotes(e.target.value)} placeholder="describe how you feel..." style={{ resize: 'none', marginBottom: 0 }} />
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px' }}>
                    <button className={styles['pt-submit']} onClick={handleSaveEditPain} disabled={!editPainType || savingEditPain}
                      style={{ background: editPainType ? 'linear-gradient(135deg, #b8860b, #d4607a)' : 'rgba(212,96,122,0.08)', color: editPainType ? '#fff' : '#b09aa4' }}>
                      {savingEditPain ? 'saving...' : 'save changes ✨'}
                    </button>
                    {!confirmDeletePain ? (
                      <button onClick={() => setConfirmDeletePain(true)} style={{ width: '100%', padding: '11px', borderRadius: '999px', border: '1.5px solid rgba(212,96,122,0.2)', background: 'transparent', color: '#b09aa4', fontSize: '13px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>
                        <i className="ti ti-trash" style={{ marginRight: '6px' }} />delete this entry
                      </button>
                    ) : (
                      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                        ...
                        <p style={{ fontSize: '13px', color: '#8b1a35', margin: 0, fontWeight: 600, textAlign: 'center' }}>are you sure? this can&apos;t be undone.</p>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button onClick={() => setConfirmDeletePain(false)} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: '1.5px solid #eddde3', background: '#fdf8fa', color: '#b09aa4', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>cancel</button>
                          <button onClick={handleDeletePain} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: 'none', background: 'linear-gradient(135deg, #d4607a, #8b1a35)', color: 'white', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 700 }}>
                            <i className="ti ti-trash" style={{ marginRight: '4px' }} /> yes, delete
                          </button>
                        </div>
                      </motion.div>
                ...
                <motion.div className={styles['pt-onboard-card']} style={{ maxWidth: 420 }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                    <div>
                      ...
                      <h2 className={styles['pt-onboard-title']} style={{ marginBottom: 0 }}>edit <span style={{ color: '#5a8c63' }}>intimacy log</span></h2>
                    </div>
                    <button onClick={closeEditSex} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid #eddde3', background: '#fdf8fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#b09aa4', flexShrink: 0 }}>
                      <i className="ti ti-x" />
                    </button>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    <div>
                      ...
                      <textarea className={styles['pt-input']} value={editSexNotes} rows={3}
                        onChange={e => setEditSexNotes(e.target.value)} placeholder="e.g. pain, discomfort, notes..." style={{ resize: 'none', marginBottom: 0 }} />
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px' }}>
                    <button className={styles['pt-submit']} onClick={handleSaveEditSex} disabled={savingEditSex}
                      style={{ background: 'linear-gradient(135deg, #5a8c63, #9b7ec8)', color: '#fff' }}>
                      {savingEditSex ? 'saving...' : 'save changes ✨'}
                    </button>
                    {!confirmDeleteSex ? (
                      <button onClick={() => setConfirmDeleteSex(true)} style={{ width: '100%', padding: '11px', borderRadius: '999px', border: '1.5px solid rgba(212,96,122,0.2)', background: 'transparent', color: '#b09aa4', fontSize: '13px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>
                        <i className="ti ti-trash" style={{ marginRight: '6px' }} />delete this entry
                      </button>
                    ) : (
                      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                        ...
                        <p style={{ fontSize: '13px', color: '#8b1a35', margin: 0, fontWeight: 600, textAlign: 'center' }}>are you sure? this can&apos;t be undone.</p>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button onClick={() => setConfirmDeleteSex(false)} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: '1.5px solid #eddde3', background: '#fdf8fa', color: '#b09aa4', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>cancel</button>
                          <button onClick={handleDeleteSex} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: 'none', background: 'linear-gradient(135deg, #5a8c63, #8b1a35)', color: 'white', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 700 }}>
                            <i className="ti ti-trash" style={{ marginRight: '4px' }} /> yes, delete
                          </button>
                        </div>
                      </motion.div>
                ...
                <motion.div className={styles['pt-onboard-card']} style={{ maxWidth: 520 }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                    <div>
                      ...
                      <h2 className={styles['pt-onboard-title']} style={{ marginBottom: 0 }}>your <span className={styles.accent}>cycle profile</span></h2>
                    </div>
                    <button onClick={closeEditProfile} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid #eddde3', background: '#fdf8fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#b09aa4', flexShrink: 0 }}>
                      <i className="ti ti-x" />
                    </button>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '65vh', overflowY: 'auto', paddingRight: '4px' }}>
                    <div>
                      ...
                      <input type="range" className={styles['pt-range']} min={20} max={45}
                        value={editProfile.cycle_length ?? 28} onChange={e => updateEditProfile('cycle_length', Number(e.target.value))} />
                    ...
                    <div>
                      ...
                      <input type="range" className={styles['pt-range']} min={2} max={10}
                        value={editProfile.period_length ?? 5} onChange={e => updateEditProfile('period_length', Number(e.target.value))} />
                      ...
                      <div className={styles['pt-radio-group']}>
                        {[
                          ...
                          <button key={opt.key} className={styles['pt-radio-btn']}
                            ...
                            style={{ background: editProfile.cycle_regularity === opt.key ? opt.bg : 'transparent', borderColor: editProfile.cycle_regularity === opt.key ? opt.border : 'rgba(212,96,122,0.12)', color: editProfile.cycle_regularity === opt.key ? opt.c : 'var(--ink)' }}>
                            <i className={`ti ${opt.icon}`} style={{ color: opt.c }} />
                      ...
                      <div className={styles['pt-radio-group']}>
                        {[
                          ...
                          <button key={String(opt.v)} className={styles['pt-radio-btn']}
                            ...
                            style={{ background: editProfile.has_pcos_pcod === opt.v ? opt.bg : 'transparent', borderColor: editProfile.has_pcos_pcod === opt.v ? opt.border : 'rgba(212,96,122,0.12)', color: editProfile.has_pcos_pcod === opt.v ? opt.c : 'var(--ink)' }}>
                            <i className={`ti ${opt.icon}`} style={{ color: opt.c }} />
                            <div><div style={{ fontWeight: 600 }}>{opt.label}</div></div>
                          </button>
                        ))}
                        ...
                        <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                          {(['pcos', 'pcod'] as const).map(t => (
                            <button key={t} className={styles['pt-radio-btn']}
                              ...
                              style={{ flex: 1, background: editProfile.pcos_type === t ? '#f3edfb' : 'transparent', borderColor: editProfile.pcos_type === t ? '#c9b8e8' : 'rgba(212,96,122,0.12)', color: editProfile.pcos_type === t ? '#9b7ec8' : 'var(--ink)' }}>
                              <i className="ti ti-ribbon" style={{ color: '#9b7ec8' }} />
                              <div><div style={{ fontWeight: 600 }}>{t.toUpperCase()}</div></div>
                            </button>
                          ))}
                        ...
                        <div className={styles['pt-checkbox-group']} style={{ marginTop: '8px' }}>
                          {['pill','IUD (hormonal)','IUD (copper)','implant','injection','patch','ring','condoms only'].map(bc => (
                            <button key={bc} className={styles['pt-checkbox-btn']}
                              ...
                              style={{ background: editProfile.birth_control_type === bc ? '#fde8ee' : 'transparent', borderColor: editProfile.birth_control_type === bc ? '#e8a0b0' : 'rgba(212,96,122,0.12)', color: editProfile.birth_control_type === bc ? '#7a1a35' : 'var(--ink)' }}>
                              <i className={`ti ${editProfile.birth_control_type === bc ? 'ti-circle-check' : 'ti-circle'}`} style={{ color: editProfile.birth_control_type === bc ? '#d4607a' : '#b09aa4' }} />{bc}
                            </button>
                          ))}
                        </div>
                      )}
                    ...
                    <div>
                      ...
                      <input type="number" className={styles['pt-input']} placeholder="e.g. 26"
                        value={editProfile.age ?? ''} onChange={e => updateEditProfile('age', Number(e.target.value) || null)} />
                    </div>
                    <div>
                      <div className={styles['pt-input-lbl']}><i className="ti ti-pencil" /> notes (optional)</div>
                      <textarea className={styles['pt-input']} rows={3}
                        placeholder="e.g. thyroid issues, endometriosis..."
                        value={editProfile.notes ?? ''} onChange={e => updateEditProfile('notes', e.target.value)}
                        style={{ resize: 'none', marginBottom: 0 }} />
                    </div>
                  </div>
                  <button className={styles['pt-submit']} onClick={handleSaveEditProfile} disabled={savingEditProfile}
                ...
                <motion.div className={styles['pt-onboard-card']} style={{ maxWidth: 500 }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                    <div>
                      ...
                      <h2 className={styles['pt-onboard-title']} style={{ marginBottom: 0 }}>edit <span style={{ color: '#9b7ec8' }}>symptom log</span></h2>
                    </div>
                    <button onClick={closeEditSymptom} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid #eddde3', background: '#fdf8fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#b09aa4', flexShrink: 0 }}>
                      <i className="ti ti-x" />
                    </button>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', maxHeight: '65vh', overflowY: 'auto', paddingRight: '4px' }}>
                    ...
                    <div>
                      ...
                      <div className={styles['pt-chips']}>
                        {symptomList.map(s => {
                          ...
                          return (
                            <button key={s.label} className={styles['pt-chip']} onClick={() => toggleEditDSymptom(s.label)}
                              style={{ background: active ? '#f3edfb' : 'transparent', borderColor: active ? '#c9b8e8' : 'rgba(155,126,200,0.2)', color: active ? '#4a2a80' : '#b09aa4' }}>
                              <i className={`ti ${s.icon}`} style={{ color: active ? '#9b7ec8' : '#b09aa4' }} />{s.label}
                            </button>
                          )
                    ...
                    <div>
                      ...
                      <div className={styles['pt-mood-row']}>
                        {moodOpts.map(m => {
                          ...
                          return (
                            <button key={m.label} className={styles['pt-mood-btn']} onClick={() => setEditDMood(active ? null : m.label)}
                              style={{ background: active ? '#f3edfb' : 'transparent', borderColor: active ? '#c9b8e8' : 'rgba(155,126,200,0.15)', color: active ? m.c : '#b09aa4' }}>
                              <i className={`ti ${m.icon}`} style={{ color: active ? m.c : '#b09aa4' }} />{m.label}
                            </button>
                          )
                      ...
                      <div className={styles['pt-severity']}>
                        {[1,2,3,4,5].map(n => (
                          <button key={n} onClick={() => setEditDEnergy(n)}
                            style={{ width: 36, height: 36, borderRadius: '10px', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '13px', fontFamily: 'DM Sans, sans-serif', background: editDEnergy >= n ? `hsl(${140 + n * 15}, ${50 + n * 5}%, ${55 - n * 3}%)` : 'transparent', color: editDEnergy >= n ? '#fff' : '#b09aa4', outline: editDEnergy >= n ? 'none' : '1.5px solid rgba(155,126,200,0.2)' }}>
                            {n}
                          </button>
                        ))}
                        <span style={{ fontSize: '11px', color: '#b09aa4', marginLeft: '4px' }}>
                    ...
                    <div>
                      ...
                      <textarea className={styles['pt-input']} value={editDNotes} rows={2}
                        onChange={e => setEditDNotes(e.target.value)} placeholder="anything on your mind..." style={{ resize: 'none', marginBottom: 0 }} />
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px' }}>
                    <button className={styles['pt-submit']} onClick={handleSaveEditSymptom} disabled={savingEditSymptom}
                      style={{ background: 'linear-gradient(135deg, #9b7ec8, #d4607a)', color: '#fff' }}>
                      {savingEditSymptom ? 'saving...' : 'save changes ✨'}
                    </button>
                    {!confirmDeleteSym ? (
                      <button onClick={() => setConfirmDeleteSym(true)} style={{ width: '100%', padding: '11px', borderRadius: '999px', border: '1.5px solid rgba(155,126,200,0.2)', background: 'transparent', color: '#b09aa4', fontSize: '13px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>
                        <i className="ti ti-trash" style={{ marginRight: '6px' }} />delete this entry
                      </button>
                    ) : (
                      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                        style={{ padding: '14px 18px', borderRadius: '18px', background: 'rgba(155,126,200,0.06)', border: '1.5px solid rgba(155,126,200,0.2)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <p style={{ fontSize: '13px', color: '#4a2a80', margin: 0, fontWeight: 600, textAlign: 'center' }}>are you sure? this can&apos;t be undone.</p>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button onClick={() => setConfirmDeleteSym(false)} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: '1.5px solid #eddde3', background: '#fdf8fa', color: '#b09aa4', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600 }}>cancel</button>
                          <button onClick={handleDeleteSymptom} style={{ flex: 1, padding: '10px', borderRadius: '999px', border: 'none', background: 'linear-gradient(135deg, #9b7ec8, #4a2a80)', color: 'white', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 700 }}>
                            <i className="ti ti-trash" style={{ marginRight: '4px' }} /> yes, delete
                          </button>
                        </div>
                      </motion.div>
